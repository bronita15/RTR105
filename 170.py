# Faisls : 170.py
# Autors:
# Apliecibas numurs:
# Datums:
# Sagatave funkcijas saknes mekleesanai ar dihatomijas metodi

# -*- coding: utf -8 -*-
from math import sin, fabs
from time import sleep

def f(x) :
    return sin(x)

# Definejam argumenta x rebezas:
a = 1.1
b = 3.2

# Aprekinam funkcijas vertibas dotajos punktos:
funa = f(a)
funb = f(b)

# Paarbaudam< vai dotajaa intervalaa ir saknes:
if (funa * funb > 0.0):
    print("Dotajaa intervalaa [%s, %s] saknju nav"%(a,b))
    sleep(1); exit()   # Zinjo uz ekraana, gaida 1 sec. un darbu pabeidz
else:
    print("Dotajaa intervaalaa sakne(s) ir!")

# Defineejam precizitaati, ar kaadu mekleesim sakni:
deltax = 0.01

# Sashaurinam saknes mekleeshanas robezas:
while (fabs(b-a) > deltax):
    x = (a+b)/2; funx = f(x)
    if (funa * funx < 0. ):
        b = x
    else:
        a = x

print("Saknes ir:", x)
